...
export async function fairSummary(fairsService: FairsService, fairId: string) {
...
export async function getFairComparisonData(
  fairsService: FairsService,
  dashboardService: DashboardService,
  chartsService: ChartsService,
  revenueChartsService: RevenueChartsService,
  fairId: string,
  compareToFairId: string,
) {
  ...
  const channels = Array.from(channelNames).map((name) => {
    ...
    return {
      ...
      delta:
        cur && base
          ? {
              totalRegistered: delta(cur.totalRegistered, base.totalRegistered),
              conversionRate: delta(cur.conversionRate, base.conversionRate),
            }
          : null,
    };
  });
...
export async function getMarketingRecommendationsData(
  fairsService: FairsService,
  dashboardService: DashboardService,
  ...
  const [fair, performance, cost] = await Promise.all([
    fairSummary(fairsService, fairId),
    getChannelPerformanceData(dashboardService, fairId),
    getChannelCostEffectivenessData(dashboardService, expensesService, fairId),
  ]);
  ...
  const avgCpa =
    channelsWithCpa.length > 0
      ? channelsWithCpa.reduce((s, c) => s + c.cpa, 0) / channelsWithCpa.length
      : null;
  ...
  const recommendations = performance.channels.map((p) => {
    ...
    const cheapOrUnknownCost = cpa === null || avgCpa === null || cpa <= avgCpa;
  ...
  if (!previousFair) {
    ...
    const channels = rampUp.map((r) => {
      ...
      const daysBeforeEvent = Math.round(
        (previousStart.getTime() - firstRegistration.getTime()) / 86_400_000,
      );
      const suggestedStartDate =
        fair.startDate && daysBeforeEvent >= 0
          ? new Date(
              new Date(fair.startDate).getTime() - daysBeforeEvent * 86_400_000,
            )
          : null;

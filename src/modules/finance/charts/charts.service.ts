...
export interface FairKpi {
  ...
  visitantes: {
    ...
    custoPorVisitante: number;   // R$ (baseado em despesas totais)
  };
}
...
export class ChartsService {
  ...
  private async totalExpenses(fairId: string) {
    ...
    return { total: r2(diretas + rateadas), diretas: r2(diretas), rateadas: r2(rateadas) };
  }
  ...
  async fairKpi(fairId: string): Promise<FairKpi> {
    const [revenues, paidRow, overdueRow, expenses, visitors, checkins] =
      await Promise.all([
        ...
        this.installmentRepo
          ...
          .innerJoin('i.revenue', 'r')
          .select('SUM(i.valueCents)', 'total')
          .where('r.fairId = :fairId AND i.status = :st', { fairId, st: 'VENCIDA' })
          .getRawOne(),
    ...
    const totalContrato = r2(revenues.reduce((s, rv) => s + Number(rv.contractValue) / 100, 0));
    ...
    const inadimplencia = totalContrato > 0 ? r2((totalVencido / totalContrato) * 100) : 0;

    const lucroProjetado  = r2(totalContrato - expenses.total);
    const lucroRealizado  = r2(totalRecebido - expenses.total);
    const margemProjetada = totalContrato > 0 ? r2((lucroProjetado / totalContrato) * 100) : 0;
    const margemRealizada = totalRecebido > 0 ? r2((lucroRealizado  / totalRecebido)  * 100) : 0;

    const taxaComparecimento = visitors > 0 ? r2((checkins / visitors) * 100) : 0;
    const custoPorVisitante  = visitors > 0 ? r2(expenses.total / visitors) : 0;

    return {
      receita:   { totalContrato, totalRecebido, totalAReceber, totalVencido, inadimplencia },
      despesas:  { total: expenses.total, diretas: expenses.diretas, rateadas: expenses.rateadas },
      resultado: { lucroProjetado, lucroRealizado, margemProjetada, margemRealizada, isProfitable: lucroProjetado > 0 },
      visitantes:{ total: visitors, checkins, taxaComparecimento, custoPorVisitante },
    };
  }
  ...
  async revenuesByStatus(fairId: string): Promise<ApexDonutData> {
    ...
    const order = ['PAGO', 'EM_ANDAMENTO', 'PENDENTE', 'EM_ATRASO', 'CANCELADO'];
    const rowMap = new Map(rows.map(r => [r.status as string, Number(r.total) / 100]));
  ...
  async revenueForecast(fairId: string): Promise<ApexBarData> {
    ...
    for (const r of rows) {
      if (!monthMap.has(r.month)) monthMap.set(r.month, { aVencer: 0, vencida: 0 });
    ...
    return {
      series: [
        { name: 'A Vencer',  data: months.map(m => r2(monthMap.get(m)!.aVencer)) },
        { name: 'Em Atraso', data: months.map(m => r2(monthMap.get(m)!.vencida)) },
      ],
  ...
  async visitorsTimeline(fairId: string): Promise<ApexBarData> {
    ...
    const cumul      = daily.map(c => { cumulative += c; return cumulative; });
  ...
  async checkinsByHour(fairId: string): Promise<ApexBarData> {
    ...
    return {
      series: [{ name: 'Check-ins', data: hours.map(h => hourMap.get(h) ?? 0) }],
  ...
  async compareRevenueVsExpenses(fairIds: string[]): Promise<ApexBarData> {
    ...
    const data = await Promise.all(
      ordered.map(async fairId => {
        ...
        const receita = r2(revenues.reduce((s, rv) => s + Number(rv.contractValue) / 100, 0));
        return { receita, despesa: expenses.total, lucro: r2(receita - expenses.total) };
      }),
    );
  ...
  async compareMargins(fairIds: string[]): Promise<ApexBarData> {
    ...
    const data = await Promise.all(
      ordered.map(async fairId => {
        ...
        const receita = revenues.reduce((s, rv) => s + Number(rv.contractValue) / 100, 0);

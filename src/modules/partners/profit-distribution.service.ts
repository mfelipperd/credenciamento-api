...
export class ProfitDistributionService {
  ...
  async distributeProfit(fairId: string, totalProfit: number): Promise<void> {
    this.logger.log(
      `Iniciando distribuição de lucro para feira ${fairId}: R$ ${totalProfit.toFixed(2)}`,
    );
    ...
    if (totalPercentage > 100) {
      this.logger.error(
        `Soma das porcentagens excede 100%: ${totalPercentage}%`,
      );
      throw new Error(
        'A soma das porcentagens dos sócios não pode exceder 100%',
      );
    }
    ...
    for (const fairPartner of fairPartners) {
      if (fairPartner.percentage > 0) {
        ...
        await this.fairPartnersService.updateEarnings(
          fairId,
          fairPartner.partnerId,
          partnerShare,
        );
